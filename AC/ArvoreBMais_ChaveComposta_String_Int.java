...
public class ArvoreBMais_ChaveComposta_String_Int {
    ...
    private int[] read1(String chave1, long pagina) throws IOException {
        ...
        if (i < pa.n && pa.filhos[0] == -1 && chave1.compareTo(pa.chaves1[i]) == 0) {
    ...
    public boolean create(String c1, int c2) throws IOException {
        ...
        // e chave2Aux. Quando há uma divisão, a chave e o valor promovidos são
        // armazenados
        // nessas variáveis.
        chave1Aux = c1;
        chave2Aux = c2;
    ...
    private boolean create1(long pagina) throws IOException {
        ...
        int i = 0;
        while (i < pa.n && (chave1Aux.compareTo(pa.chaves1[i]) > 0
                || (chave1Aux.compareTo(pa.chaves1[i]) == 0 && chave2Aux > pa.chaves2[i]))) {
            i++;
        }
        ...
        boolean inserido;
        if (i == pa.n || chave1Aux.compareTo(pa.chaves1[i]) < 0
                || (chave1Aux.compareTo(pa.chaves1[i]) == 0 && chave2Aux < pa.chaves2[i]))
        ...
        // Caso 2 - Novo registro deve ficar na página da direita
            ...
            int j;
            for (j = maxElementos - meio; j > 0 && (chave1Aux.compareTo(np.chaves1[j - 1]) < 0
                    || (chave1Aux.compareTo(np.chaves1[j - 1]) == 0 && chave2Aux < np.chaves2[j - 1])); j--) {
    ...
    private boolean delete1(String chave1, int chave2, long pagina) throws IOException {
        ...
        int i = 0;
        while (i < pa.n && (chave1.compareTo(pa.chaves1[i]) > 0
                || (chave1.compareTo(pa.chaves1[i]) == 0 && chave2 > pa.chaves2[i]))) {
            i++;
        }
    ...
    private void print1(long pagina) throws IOException {
        ...
        for (i = 0; i < maxElementos; i++) {
            System.out.print("(" + String.format("%04d", pa.filhos[i]) + ") " + pa.chaves1[i] + ","
